...
public class BasePlatformConfiguration implements EnvironmentAware {
	...
	private void initialize() {
		...
		Properties avProperties = new Properties();
		PlatformPropertiesUtil.getMandatoryAVProperties().stream().forEach(property -> {
			avProperties.put(property, PropertiesUtil.getProperty(property));
		});
		platformConfig = new PlatformConfiguration.Builder().withFrontDoorProperties(frontDoorProperties)
				...
				.withAppName(PropertiesUtil.getProperty("spring.application.name")).isMultitenantApp(true)
				.withAVProperties(avProperties).build();
	}
